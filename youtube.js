...
function captionsToText(captions) {
  return captions
    .map(
      (c) =>
        `[${formatTimestamp(c.start)} | ${Math.floor(c.start)}s] ${c.text}`,
    )
    .join("\n");
}
...
async function fetchVideoData(videoId) {
  ...
  try {
    // Get metadata
    const { stdout: jsonStr } = await execFileAsync(
      "uvx",
      [
        "yt-dlp",
        ...cookiesArgs,
    ...
    if (!subLang) {
      ...
      if (origLang && autoLangs.includes(origLang)) {
        ...
        subLang = autoLangs.includes("cs")
          ? "cs"
          : autoLangs.includes("en")
            ? "en"
            : autoLangs[0] || "";
      }
      useAuto = true;
...
async function fetchChannelInfo(channelId) {
  ...
  if (avatarMatch) {
    avatarUrl = avatarMatch[1].replace(
      /=s\d+[^"]*$/,
...
async function fetchChannelId(videoId) {
  ...
  try {
    const cookiesFile = path.join(__dirname, "data", "cookies.txt");
    const cookiesArgs = fs.existsSync(cookiesFile)
      ? ["--cookies", cookiesFile]
      : [];
